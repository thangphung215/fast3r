    ...
    def run(self):
        ...
        while True:
            ...
            if cmd["cmd"] == "launch":
                ...
                self.next_server_id += 1
                session_id = cmd.get(
                    "session_id", f"default_session_{server_id}")
                ...
                try:
                    ...
                    if "error" in result:
                        ...
                        self.servers[server_id] = {
                            "share_url": share_url, "process": p, "session_id": session_id}
                ...
                if server_id in self.servers:
                    ...
                    try:
                        ...
                        if session_id in self.session_servers and server_id in self.session_servers[session_id]:
                            ...
                            # If empty list
                            if not self.session_servers[session_id]:
                    ...
                    self.console.log(
                        f"[red]Server with id {server_id} not found[/red]")
                    self.resp_queue.put({
                        "cmd": "terminate_server",
                        "server_id": server_id,
                        "error": "ID not found",
                ...
                if session_id in self.session_servers:
                    ...
                    server_ids = self.session_servers[session_id].copy()
                    self.console.log(
...
def start_manager():
    ...
    manager_process = mp.Process(
        target=ViserServerManager(req_queue, resp_queue).run)
...
def create_demo(checkpoint_dir, examples_dir, output_dir, device: torch.device, is_lightning_checkpoint=False):
    ...
    with gr.Blocks(title="Fast3R 3D Reconstruction Demo", ) as demo:
        ...
        """
        state = gr.State(value={"session_id": "", "urls": []},
                         delete_callback=delete_visers_callback)

        gr.HTML(header_html)
        with gr.Row():
            with gr.Column(scale=2):
                gallery = gr.Gallery(label="Upload Images of a Scene",
                                     columns=6, height="150px", show_download_button=True)
                video_input.render()
            with gr.Column(scale=1):
                ...
                submit_button = gr.Button(
                    "Submit", variant="primary", size="lg")
        ...
        # loading animation (full row during loading)
        loading_message = gr.HTML("")
        with gr.Row():
            with gr.Column(scale=2):
                # For visualization ready message
                vis_message = gr.HTML(visible=False)
            with gr.Column(scale=1):
                with gr.Column(visible=False, elem_classes="floating-box") as feedback_column:
                    ...
                    with gr.Row():
                        thumbs_up = gr.Button("👍 Love it!", elem_classes=[
                                              "feedback-button", "positive"])
                        thumbs_down = gr.Button("👎 Not quite there", elem_classes=[
                                                "feedback-button", "negative"])

        # 3D viewer iframe (full row final)
        viser_iframe = gr.HTML("")
        ...
        video_input.change(fn=update_video_upload,
                           inputs=[gallery, video_input,
                                   just_uploaded_video, state],
        ...
        def process_images_wrapper(uploaded_files, video_file, state,
            ...
            for output in generator:
                ...
                yield (loading_message, vis_message, feedback_column,
                       viser_iframe, status_box, state,
                       gr.update(visible=True), gr.update(visible=True),
                       gr.update(visible=False))  # make feedback buttons visible and thank you message invisible
        ...
        submit_button.click(
            ...
            outputs=[loading_message, vis_message, feedback_column,
                     viser_iframe,
                     status_box, state, thumbs_up, thumbs_down, thank_you],
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fast3R 3D Reconstruction Demo...")
    ...
    parser.add_argument("--output_dir", type=str, default="./demo_outputs",
                        help="Dir to store processed scenes with feedback")
    parser.add_argument("--is_lightning_checkpoint", action="store_true",
                        default=False,
                        help="Whether the checkpoint is from Lightning \
    ...
    demo = create_demo(args.checkpoint_dir, args.examples_dir, args.output_dir,
                       device=device,
