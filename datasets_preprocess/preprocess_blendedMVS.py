...
def main(db_root, pairs_path, output_dir):
    ...
    for i, seq in enumerate(tqdm(sequences)):
        ...
        func_args = [(root, f[:-8], out_dir)
                     for f in os.listdir(cam_dir) if not f.startswith('pair')]
        parallel_threads(load_crop_and_save, func_args,
                         star_args=True, leave=False)
...
def load_crop_and_save(root, img, out_dir):
    ...
    depthmap_in = load_pfm_file(
        osp.join(root, 'rendered_depth_maps', img + '.pfm'))
