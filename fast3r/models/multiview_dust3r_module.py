...
def gather_deduplicated_scene_metrics(reconstruction_metrics_per_epoch):
    """Gathers and deduplicates scene-specific metrics across all ranks by 
    dataset."""
    ...
    for rank_metrics in gathered_metrics:
        for dataset_name, scenes in rank_metrics.items():  # type: ignore
            if dataset_name not in all_metrics:
                all_metrics[dataset_name] = {}
            # Keeps the first occurrence of each scene
            all_metrics[dataset_name].update(scenes)
...
class MultiViewDUSt3RLitModule(LightningModule):
    def __init__(
        ...
        # so that when we resume training, these bookkeeping variables are
        # preserved
        self.register_buffer("epoch_fraction", torch.tensor(
            0.0, dtype=torch.float32, device=self.device))
        self.register_buffer("train_total_samples", torch.tensor(
            0, dtype=torch.long, device=self.device))
        self.register_buffer("train_total_images", torch.tensor(
            0, dtype=torch.long, device=self.device))

        # these need to be reduced across GPUs, so use Metric
        self.train_total_samples_per_step = AccumulatedSum()
        # these need to be reduced across GPUs, so use Metric
        self.train_total_images_per_step = AccumulatedSum()
        ...
        self.dataset_names_with_samples_of_uneven_num_of_views = [
            'dtu', '7scenes', 'nrgbd']
        # Accumulate all reconstruction metrics by dataset and scene for the
        # epoch
        self.reconstruction_metrics_per_epoch = {}
        # New dictionary to store detailed losses for datasets with uneven
        # number of views
        self.uneven_view_detailed_losses = {}

    @classmethod
    def load_for_inference(cls, net: LightFast3Rv2):
        lit_module = cls(net=net,
    ...
    def on_train_start(self) -> None:
        ...
        # training starts,
        # so it's worth to make sure validation metrics don't store results
        # from these checks
        self.val_loss.reset()
    ...
    def on_train_epoch_start(self) -> None:
        # our custom dataset and sampler has to have epoch set by calling
        # set_epoch
        if hasattr(
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        for loader in self.trainer.val_dataloaders:
            if hasattr(
                    loader, "dataset") and hasattr(
                        loader.dataset, "set_epoch"):
                loader.dataset.set_epoch(0)
            if hasattr(
                    loader, "sampler") and hasattr(
    ...
    def model_step(
        ...
        # Move data to device
        # list_names = "img pts3d valid_mask camera_pose camera_intrinsics
        # F_matrix corres".split()
        list_names = ["img", "pts3d", "valid_mask", "camera_pose",
                      "camera_intrinsics", "F_matrix", "corres"]
        for view in batch:
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float32):
            loss, loss_details = criterion(
                views, preds) if criterion is not None else None
    ...
    def training_step(
        ...
        views, preds, loss, loss_details = self.model_step(
            batch, self.train_criterion)

        # this will cause a
        # lightning.fabric.utilities.exceptions.MisconfigurationException
        if not isinstance(loss, (torch.Tensor, dict, type(None))):
            # log loss and the batch information to help debugging
            # use print instead of log because the logger only logs on rank 0,
            # but this could happen on any rank
            print(
                f"Loss's not a tensor or dict but {type(loss)}, value: {loss}")
            print(f"Loss details: {loss_details}")
            print(f"Batch: {batch}")
            print(f"Batch index: {batch_idx}")
            print(f"Views: {views}")
            print(f"Preds: {preds}")
            # set loss to None will still break the training loop in DDP, this
            # is intended - we should fix the data to avoid nan loss in the
            # first place
            loss = None
            return loss

        temp_frac = self.trainer.current_epoch
        temp_frac += batch_idx / self.trainer.num_training_batches
        self.epoch_fraction = torch.tensor(temp_frac, device=self.device)

        self.log("trainer/epoch", self.epoch_fraction,
                 on_step=True, on_epoch=False, prog_bar=True)
        tmp_lr = self.trainer.lr_scheduler_configs[0].scheduler.get_last_lr()[
            0]
        self.log("trainer/lr", tmp_lr, on_step=True,
        ...
        if loss_details is not None:
            for key, value in loss_details.items():
                self.log(f"train_detail_{key}", value,
                         on_step=True, on_epoch=False, prog_bar=False)
                match = re.search(r'/(\d{1,2})$', key)
                if match:
                    stripped_key = key[:match.start()]
                    self.log(f"train/{stripped_key}", value,
                             on_step=True, on_epoch=False, prog_bar=False)

        # Log the total number of samples seen so far
        batch_size = views[0]["img"].shape[0]  # type: ignore
        self.train_total_samples_per_step(
            batch_size)  # aggregate across all GPUs
        # accumulate across all steps
        self.train_total_samples += self.train_total_samples_per_step.compute()
        ...
        self.train_total_images_per_step(
            n_image_cur_step)  # aggregate across all GPUs
        # accumulate across all steps
        self.train_total_images += self.train_total_images_per_step.compute()
    ...
    def validation_step(
        ...
        # all views should have the same dataset name because we use
        # "sequential" mode of CombinedLoader
        dataset_name = views[0]['dataset'][0]
        batch_size = views[0]["img"].shape[0]

        # Log the overall validation loss
        # self.log("val/loss", loss, on_step=False, on_epoch=True,
        # prog_bar=True, reduce_fx="mean", sync_dist=True,
        # add_dataloader_idx=True, batch_size=batch_size)
        self.val_loss(loss)
        # self.log(f"val/loss_{dataset_name}", loss, on_step=False,
        # on_epoch=True, prog_bar=True, reduce_fx="mean", sync_dist=True,
        # add_dataloader_idx=False, batch_size=batch_size)

        # Log the details of the loss with dataset name and view number in the
        # key

        if loss_details is not None:
            dsnames = self.dataset_names_with_samples_of_uneven_num_of_views
            if dataset_name not in dsnames:
                for key, value in loss_details.items():
                    self.log(
                    ...
                    if match:
                        stripped_key = key[:match.start()]
                        self.log(f"val/{dataset_name}_{stripped_key}",
                                 value, on_step=False, on_epoch=True,
                                 prog_bar=False, reduce_fx="mean",
                                 sync_dist=True, add_dataloader_idx=False,
                                 batch_size=batch_size)
            else:
                # if the dataset name is in
                # self.dataset_names_with_samples_of_uneven_num_of_views,
                # do not use self.val_loss but log it to the holder so that we
                # can use a custom aggregation logic to reduce the loss
                # this is because the there are different number of views per
                # sample in these datasets, but self.log assumes that all
                # samples have the same number of views,
                # otherwise self.log will enter a deadlock because it will
                # wait for the same number of samples from all ranks

                # Store in dictionary instead of logging directly
                for key, value in loss_details.items():
                    temp_loss = self.uneven_view_detailed_losses
                    if dataset_name not in temp_loss:
                        temp_loss[dataset_name] = {}
                    new_key = f"val_detail_{dataset_name}_{key}"
                    if new_key not in temp_loss[dataset_name]:
                        temp_loss[dataset_name][new_key] = [
                        ]
                    temp_loss[dataset_name][new_key].append(value)
        ...
        if dataset_name == "Co3d_v2":
            self.evaluate_camera_poses(
                views, preds, niter_PnP=100,
                focal_length_estimation_method='first_view_from_global_head')

        # Evaluate point clouds only for the reconstruction datasets
        # (DTU, 7-Scenes, and NRGBD)
        ...
        if dataset_name in ['dtu', '7scenes', 'nrgbd']:
            if (self.current_epoch % 5 == 4 or self.current_epoch == 0):
                ...
                # use only the very confident points for alignment and
                # use all of the points for metric calculation
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # if we dont do these, wandb for some reason cannot display the
        # validation loss with them as the x-axis
        self.log("trainer/epoch", self.epoch_fraction, sync_dist=True)
        self.log("trainer/total_samples",
                 self.train_total_samples.cpu().item(), sync_dist=True)
        self.log("trainer/total_images",
                 self.train_total_images.cpu().item(), sync_dist=True)
    ...
    def aggregate_and_log_reconstruction_detail_losses(self):
        ...
        # Gather and aggregate detailed losses for uneven-view datasets across
        # all ranks
        if torch.distributed.is_initialized():
            if self.global_rank == 0:
                ...
                gathered_detailed_losses = None
            # all_gather_object(gathered_detailed_losses, 
            # self.uneven_view_detailed_losses)
    ...
    def aggregate_and_log_reconstruction_metrics(self):
        ...
        for dataset_name, scenes in self.reconstruction_metrics_per_epoch.items():
            for scene_name, metrics in scenes.items():
                self.log(f"val_recon_{dataset_name}_detail/{scene_name}/accuracy",
                         metrics["accuracy"], sync_dist=True)
                self.log(f"val_recon_{dataset_name}_detail/{scene_name}/accuracy_median",
                         metrics["accuracy_median"], sync_dist=True)
                self.log(f"val_recon_{dataset_name}_detail/{scene_name}/completion",
                         metrics["completion"], sync_dist=True)
                self.log(f"val_recon_{dataset_name}_detail/{scene_name}/completion_median",
                         metrics["completion_median"], sync_dist=True)
                self.log(
                    f"val_recon_{dataset_name}_detail/{scene_name}/nc1", metrics["nc1"], sync_dist=True)
                self.log(f"val_recon_{dataset_name}_detail/{scene_name}/nc1_median",
                         metrics["nc1_median"], sync_dist=True)
                self.log(
                    f"val_recon_{dataset_name}_detail/{scene_name}/nc2", metrics["nc2"], sync_dist=True)
                self.log(f"val_recon_{dataset_name}_detail/{scene_name}/nc2_median",
                         metrics["nc2_median"], sync_dist=True)

        # Aggregate global metrics per dataset using deduplicated data
        for dataset_name, scenes in self.reconstruction_metrics_per_epoch.items():
            acc_list = [metrics["accuracy"] for metrics in scenes.values()]
            acc_med_list = [metrics["accuracy_median"]
                            for metrics in scenes.values()]
            comp_list = [metrics["completion"] for metrics in scenes.values()]
            comp_med_list = [metrics["completion_median"]
                             for metrics in scenes.values()]
            nc1_list = [metrics["nc1"] for metrics in scenes.values()]
            nc1_med_list = [metrics["nc1_median"]
                            for metrics in scenes.values()]
            nc2_list = [metrics["nc2"] for metrics in scenes.values()]
            nc2_med_list = [metrics["nc2_median"]
                            for metrics in scenes.values()]
            ...
            self.log(f"val_recon_{dataset_name}/accuracy",
                     mean_accuracy, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/accuracy_median",
                     median_accuracy, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/completion",
                     mean_completion, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/completion_median",
                     median_completion, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/nc1", mean_nc1, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/nc1_median",
                     median_nc1, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/nc2", mean_nc2, sync_dist=True)
            self.log(f"val_recon_{dataset_name}/nc2_median",
                     median_nc2, sync_dist=True)
    ...
    def align_local_pts3d_to_global(self, preds, views, min_conf_thr_percentile=0):
        ...
        for pred in preds:
            ...
            if 'pts3d_in_other_view' not in pred:
                raise ValueError(
                    "Key 'pts3d_in_other_view' not found in preds.")
            if 'conf' not in pred:
                raise ValueError(
        ...
        B, H, W, _ = preds[0]['pts3d_local'].shape
        ...
        def process_view_batch(view_index, batch_index):
            ...
            # Shape: (H, W, 3)
            pts3d_local = pred['pts3d_local'][batch_index]
            # Shape: (H, W)
            conf_local = pred['conf_local'][batch_index]
            # Shape: (H, W, 3)
            pts3d_global = pred['pts3d_in_other_view'][batch_index]
            # Shape: (H, W)
            conf_global = pred['conf'][batch_index]
            ...
            if x.shape[0] < 3:
                ...
                R = torch.eye(3, device=pts_local_flat.device,
                              dtype=pts_local_flat.dtype)
                t = torch.zeros(3, device=pts_local_flat.device,
                                dtype=pts_local_flat.dtype)
                s = 1.0
            else:
            ...
            pts_local_aligned_flat = s * \
                (pts_local_flat @ R.T) + t  # Shape: (N, 3)
        ...
        tasks = [(view_idx, batch_idx) for view_idx in range(num_views)
                 for batch_idx in range(B)]

        # Use ThreadPoolExecutor to parallelize across tasks
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(
                process_view_batch, view_idx, batch_idx) for view_idx, batch_idx in tasks]
        ...
        aligned_pts_dict = {view_idx: [None] *
                            B for view_idx in range(num_views)}
    ...
    def evaluate_reconstruction(self, views, preds, dataset_name,
        ...
        if use_pts3d_from_local_head:
            self.align_local_pts3d_to_global(
                preds, views, min_conf_thr_percentile=min_conf_thr_percentile_for_local_alignment_and_icp)
        ...
        assert min_conf_thr_percentile_for_local_alignment_and_icp >= min_conf_thr_percentile_for_metric_cacluation

        # Define the function to process a single sample
        def process_sample(i):
            scene_name = "/".join(views[i]['label'][0].split('/')
                                  [:-1]) if "label" in views[i] else "unknown"
            pred_pts_list = []
            gt_pts_list_icp = []
            gt_pts_list_metrics = []
            colors_pred_list = []
            ...
            for j, (view, pred) in enumerate(zip(views, preds)):
                ...
                # Shape: (H, W, 3)
                pts_pred = pred['pts3d_local_aligned_to_global'][i] if use_pts3d_from_local_head else pred['pts3d_in_other_view'][i]
                # Shape: (H, W)
                conf = pred['conf_local'][i] if use_pts3d_from_local_head else pred['conf'][i]
                ...
                conf_threshold_value_metric_calc = torch.quantile(
                    conf_flat, min_conf_thr_percentile_for_metric_cacluation / 100.0)  # Metrics should use all valid points
                conf_mask_metric_calc = conf >= conf_threshold_value_metric_calc

                # Create masks
                # Predicted points: valid and high-conf points
                final_mask_pred = valid_mask & conf_mask_metric_calc
                # GT points for ICP: all valid and high-conf points
                final_mask_gt_icp = valid_mask & conf_mask_metric_calc
                # GT points for metrics: all valid points
                final_mask_gt_metrics = valid_mask

                # Apply masks to predicted points and conf
                # High-confidence predicted points
                pts_pred_masked = pts_pred[final_mask_pred]
                # Corresponding confidence values
                conf_masked = conf[final_mask_pred]

                # Apply mask to GT points for ICP
                # GT points corresponding to high-confidence predicted points
                pts_gt_masked_icp = pts_gt[final_mask_gt_icp]

                # Apply mask to GT points for metrics
                # All valid GT points in this view
                pts_gt_masked_metrics = pts_gt[final_mask_gt_metrics]
                ...
                colors_pred_masked = img[final_mask_pred]
                # Colors at all valid GT points
                colors_gt_masked = img[final_mask_gt_metrics]
                ...
                conf_threshold_value_for_icp = torch.quantile(
                    conf_flat, min_conf_thr_percentile_for_local_alignment_and_icp / 100.0)  # ICP should use high-confidence points

                # Shape: (H, W)
                weights_masked = conf_masked >= conf_threshold_value_for_icp
            ...
            # Shape: (N_pred, 3)
            pred_pts_all = torch.cat(pred_pts_list, dim=0)
            gt_pts_all_icp = torch.cat(
                gt_pts_list_icp, dim=0)       # Shape: (N_pred, 3)
            gt_pts_all_metrics = torch.cat(
                gt_pts_list_metrics, dim=0)  # Shape: (N_gt, 3)
            # Shape: (N_pred, 3)
            colors_pred_all = torch.cat(colors_pred_list, dim=0)
            # Shape: (N_gt, 3)
            colors_gt_all = torch.cat(colors_gt_list, dim=0)
            # Shape: (N_pred,)
            ...
            # High-confidence predicted points (N_pred, 3)
            x = pred_pts_tensor
            ...
            gt_pcd.points = o3d.utility.Vector3dVector(
                gt_pts_tensor_metrics.numpy())
            gt_pcd.colors = o3d.utility.Vector3dVector(
                colors_gt_tensor.numpy())
            ...
            acc, acc_med, nc1, nc1_med = accuracy(
                gt_points_np, pred_points_np, gt_normals, pred_normals, device=views[
                    i]['pts3d'].device
            )
        ...
        with ThreadPoolExecutor() as executor:
            results = [future.result() for future in [executor.submit(
                process_sample, i) for i in range(batch_size)]]
        ...
        for result in results:
            ...
            self.reconstruction_metrics_per_epoch[dataset_name].update(
                result)  # Accumulate per dataset for the epoch
    ...
    def evaluate_camera_poses(self, views, preds, niter_PnP=10, focal_length_estimation_method='individual'):
        ...
        poses_c2w_estimated, estimated_focals = self.estimate_camera_poses(
            preds=preds, views=views, niter_PnP=niter_PnP, focal_length_estimation_method=focal_length_estimation_method)
        ...
        device = self.device
        pred_cameras = torch.tensor(np.stack(
            poses_c2w_estimated), dtype=poses_c2w_gt[0].dtype, device=device)  # Shape (B, num_views, 4, 4)
        gt_cameras = torch.stack(poses_c2w_gt).transpose(
            0, 1)  # (B, num_views, 4, 4)
        ...
        if pred_cameras.shape[1] >= 2:
            ...
            def process_sample(sample_idx):
                ...
                # Shape (num_views, 4, 4)
                gt_sample = gt_cameras[sample_idx]
                ...
                results['mAA_30'] = calculate_auc(
                    rel_rangle_deg, rel_tangle_deg, max_threshold=30).item()
            ...
            with ThreadPoolExecutor() as executor:
                batch_results = list(executor.map(
                    process_sample, range(batch_size)))

            # Update metrics for all samples in the batch
            for results in batch_results:
                for tau in self.RRA_thresholds:
                    getattr(self, f'val_RRA_{tau}')(results[f"RRA_at_{tau}"])
                    self.log(f"val_metric/RRA_at_{tau}", getattr(self, f'val_RRA_{tau}'), on_step=False, on_epoch=True,
                             prog_bar=True, reduce_fx="mean", sync_dist=True, add_dataloader_idx=False, batch_size=batch_size)
                for tau in self.RTA_thresholds:
                    getattr(self, f'val_RTA_{tau}')(results[f"RTA_at_{tau}"])
                    self.log(f"val_metric/RTA_at_{tau}", getattr(self, f'val_RTA_{tau}'), on_step=False, on_epoch=True,
                             prog_bar=True, reduce_fx="mean", sync_dist=True, add_dataloader_idx=False, batch_size=batch_size)
                self.val_mAA(results['mAA_30'])
                self.log("val_metric/mAA_30", self.val_mAA, on_step=False, on_epoch=True, prog_bar=True,
                         reduce_fx="mean", sync_dist=True, add_dataloader_idx=False, batch_size=batch_size)
    ...
    def estimate_camera_poses(preds, views=None, niter_PnP=10, focal_length_estimation_method='individual'):
        ...
        for i in range(batch_size):
            ...
            sample_preds = [{key: value[i].cpu()
                             for key, value in view.items()} for view in preds]
        ...
        def estimate_focal_for_sample(sample_preds):
            if focal_length_estimation_method == 'first_view_from_global_head':
                ...
                pts3d_i = sample_preds[0]["pts3d_in_other_view"].unsqueeze(
                    0)  # Shape: (1, H, W, 3)
                conf_i = sample_preds[0]["conf"].unsqueeze(
                    0)                  # Shape: (1, H, W)
                ...
                pts3d_i = sample_preds[0]["pts3d_local_aligned_to_global"].unsqueeze(
                    0)  # Shape: (1, H, W, 3)
                conf_i = sample_preds[0]["conf_local"].unsqueeze(
                    0)                       # Shape: (1, H, W)
        ...
        with ThreadPoolExecutor() as executor:
            data_for_processing = list(executor.map(
                estimate_focal_for_sample, data_for_processing))
        ...
        estimate_cam_pose_one_sample_partial = partial(
            estimate_cam_pose_one_sample, niter_PnP=niter_PnP, min_conf_thr_percentile=85)

        with ThreadPoolExecutor() as executor:
            results = list(executor.map(
                estimate_cam_pose_one_sample_partial, data_for_processing))
                    ...
                    for i in range(view["true_shape"].shape[0]):
                        ...
                        if H > W:
                            ...
                            transposed_pts3d_local = pred["pts3d_local"][i].transpose(
                                0, 1)
                            if "pts3d_local_aligned_to_global" in pred:
                                transposed_pts3d_local_aligned_to_global = pred["pts3d_local_aligned_to_global"][i].transpose(
                                    0, 1)
    ...
    def configure_optimizers(self) -> Dict[str, Any]:
        ...
        if self.hparams.scheduler is not None:
            ...
            if scheduler_config.func is LinearWarmupCosineAnnealingLR:
                ...
                scheduler_kwargs = {k: v for k,
                                    v in scheduler_config.keywords.items()}
    ...
    def _load_pretrained_weights(self) -> None:
        ...
        if isinstance(self.net, FlashDUSt3R):
            ckpt = torch.load(self.pretrained)
            ckpt = self._update_ckpt_keys(ckpt, new_head_name='downstream_head',
                                          head_to_keep='downstream_head1', head_to_discard='downstream_head2')
            self.net.load_state_dict(ckpt["model"], strict=False)
            del ckpt  # in case it occupies memory
        elif isinstance(self.net, LightFast3Rv2):
            if self.pretrained.endswith("DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth"):
                ...
                filtered_state_dict = {
                    k: v for k, v in checkpoint['state_dict'].items() if k.startswith('net.')}
...
def estimate_cam_pose_one_sample(sample_preds, device='cpu', niter_PnP=10, min_conf_thr_percentile=0):
    ...
    def process_view(view_idx):
        pts3d = sample_preds[view_idx]["pts3d_in_other_view"].cpu(
        ).numpy().squeeze()  # (H, W, 3)
        valid_mask = sample_preds[view_idx]["conf"].cpu(
        ).numpy().squeeze() > 1.0  # Confidence mask
        ...
        focal_length = float(sample_preds[view_idx]["focal_length"]
                             ) if "focal_length" in sample_preds[view_idx] else None
        ...
        if pose_c2w is None or focal_length is None:
            ...
            # Return identity pose in case of failure
            return np.eye(4), focal_length
